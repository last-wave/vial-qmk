...
//      Ct_L     Win_L    Alt_L                               SPACE                               Alt_R    FN       Ct_R     Left     Down     Right
    ...
    [_FN1] = LAYOUT(
        ...
        _______, ALT_F4,  CTRL_W,  RGB_SPI, RGB_TOG, RGB_RMOD,RGB_HUD, RGB_SAD, RGB_VAD, RGB_SPD, KC_PSCR, KC_SLCK, KC_PAUS, RESET,            KC_MUTE,
        _______, _______, RGB_SPI, _______, RGB_TOG, RGB_MOD, RGB_HUI, RGB_SAI, RGB_VAI, RGB_SPI, _______, _______,          KC_MPLY,          KC_MUTE,
        _______,          _______, _______, _______, RGB_VAI, _______, RGB_RMOD,RGB_MOD,_______,  _______, _______,          _______, KC_VOLU, _______,
        _______, _______, KC_MPRV,                            KC_MPLY,                            KC_MNXT, _______, _______, KC_MPRV, KC_VOLD, KC_MNXT
    ),
...
// }
    ...
    void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
        ...
        if (host_keyboard_led_state().caps_lock) {
            int leds[5] = {3, 70, 73, 76, 80};
            int i;
            int size = (int)(sizeof(leds)/sizeof(leds[0]));

            for (i = 0; i < size; i++) {
                RGB_MATRIX_INDICATOR_SET_COLOR(leds[i], 255, 255, 255);
            }
        }

        // Turn on glow when FN is activated
        uint8_t layer = get_highest_layer(layer_state);
        if (layer > 0) {
            int leds[5] = {55, 71, 74, 77, 81};
            int i;
            int size = (int)(sizeof(leds)/sizeof(leds[0]));
            ...
            RGB_MATRIX_INDICATOR_SET_COLOR(layer, 255, 255, 255);
        }
    }
